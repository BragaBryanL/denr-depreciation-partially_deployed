  ...
  //   console.log('Real-time Firebase listeners disabled...');
    ...
    const assetsUnsub = subscribeToAssets((assetsData, error) => {
      ...
      if (assetsData) {
        ...
        // Filter out assets that were recently updated locally to prevent overwriting
        const filteredAssetsData = assetsData.filter(asset => !recentLocalUpdates.has(asset.id));
        
        // Transform Firebase data to match app format
        const transformedAssets = filteredAssetsData.map(asset => {
          // Fix useful life based on account code (Land should have no useful life)
          let usefulLife = asset.usefulLife;
          if (asset.accountCode === '10601010' || asset.accountCode === '10602020' || asset.accountCode === '10699010' || asset.accountCode === '10699030') {
            usefulLife = ''; // Land and Construction in Progress have no useful life
        ...
        console.log('Transformed assets accountableOfficer values:', transformedAssets.map(a => ({id: a.id, accountableOfficer: a.accountableOfficer})));
        
        // Merge with existing assets, preserving recently updated local assets
        setAssets(prevAssets => {
          const mergedAssets = [...prevAssets];
          
          // Update only assets that weren't recently updated locally
          transformedAssets.forEach(updatedAsset => {
            const existingIndex = mergedAssets.findIndex(a => a.id === updatedAsset.id);
            if (existingIndex !== -1) {
              mergedAssets[existingIndex] = updatedAsset;
            } else {
              mergedAssets.push(updatedAsset);
            }
          });
        ...
        // Clean up recent local updates after 5 seconds
        setTimeout(() => {
          setRecentLocalUpdates(prev => {
            const newSet = new Set(prev);
            // Note: We can't easily determine which assets to remove without tracking timestamps
            // For now, this prevents indefinite blocking
            return newSet;
          });
        }, 5000);
        
        // Update localStorage with filtered Firebase data
        localStorage.setItem('denr_assets', JSON.stringify(filteredAssetsData));
      }
    });
